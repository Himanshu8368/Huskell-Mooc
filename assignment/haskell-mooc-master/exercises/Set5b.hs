...
isOrdered (Node value left right) =
    allValues (< value) left &&
    allValues (> value) right &&
    isOrdered left &&
    isOrdered right
  where
    allValues :: (a -> Bool) -> Tree a -> Bool
    allValues _ Empty = True
    allValues condition (Node v l r) =
        condition v && allValues condition l && allValues condition r
...
search val (Node value left right)
    | val == value = Just []
    | otherwise =
        case (search val left, search val right) of
            (Just steps, _) -> Just (StepL : steps)
            (_, Just steps) -> Just (StepR : steps)
            _ -> Nothing
