...
dotAndLine = Picture f
  where
    f (Coord 3 4) = white   -- white dot at (3,4)
    f (Coord _ 8) = pink    -- pink line at y=8
    f _           = black   -- everywhere else is black
------------------------------------------------------------------------------
...
combine blendFn (Picture f1) (Picture f2) = Picture f
  where
    f coord = blendFn (f1 coord) (f2 coord)
...
paintSolid color shape base = Picture f
  where
    Picture baseFunc = base
    f coord@(Coord x y)
      | contains shape x y = color
      | otherwise = baseFunc coord
...
paint pat shape base = Picture f
  where
    ...
    Picture patFunc = pat
    f coord@(Coord x y)
      | contains shape x y = patFunc coord
      | otherwise = baseFunc coord
...
instance Transform Blur where
  apply Blur = blurImage

-- Helper function to blur an image
blurImage :: Picture -> Picture
blurImage (Picture f) = Picture $ \coord ->
  let Color r1 g1 b1 = f coord
      neighbors = map f (getNeighbors coord)
      totalColors = foldl addColors (Color 0 0 0) (Color r1 g1 b1 : neighbors)
      numNeighbors = length neighbors + 1  -- Including the center pixel
      avgColor = divColor totalColors numNeighbors
  in avgColor

-- Helper function to get neighboring coordinates of a given coordinate
getNeighbors :: Coord -> [Coord]
getNeighbors (Coord x y) =
  [Coord (x-1) y, Coord (x+1) y, Coord x (y-1), Coord x (y+1)]

-- Helper function to add two colors component-wise
addColors :: Color -> Color -> Color
addColors (Color r1 g1 b1) (Color r2 g2 b2) =
  Color (r1 + r2) (g1 + g2) (b1 + b2)

-- Helper function to divide each component of a color by a number
divColor :: Color -> Int -> Color
divColor (Color r g b) n =
  Color (r `div` n) (g `div` n) (b `div` n)
...
instance Transform BlurMany where
  apply (BlurMany 0) pic = pic  -- Base case: zero blurs, return the original picture
