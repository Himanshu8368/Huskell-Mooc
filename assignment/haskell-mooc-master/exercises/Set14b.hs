...
balance db name = do
  result <- query db balanceQuery [name]
  case result of
    [[sqlValue]] -> return sqlValue
    _            -> return 0
...
parseCommand [cmd, account]
  | cmd == T.pack "balance" = Just (Balance account)
parseCommand [cmd, account, amount]
  | cmd == T.pack "deposit" = parseInt amount >>= \x -> Just (Deposit account x)
