...
appendAll ref strings = do
    ...
    let updatedValue = currentValue ++ concat strings
    writeIORef ref updatedValue
...
interact' f initialState = loop initialState
  where
    loop state = do
        input <- getLine  -- Step 1: Read a line of input from the user
        let (continue, output, newState) = f (input, state)  -- Step 2: Call f with input and current state
        putStrLn output  -- Step 3: Print the output string from f
        if continue
            then loop newState  -- Step 4: Continue with the new state if continue is True
            else return newState  -- Step 4: Return the final state if continue is False
