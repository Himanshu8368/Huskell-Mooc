...
sumSuccess measurements =
  let successes = [x | Right x <- measurements]
  in if null(successes) then Left "no data"
     else Right (sum successes)
...
permute perm input = map (\i -> input !! (perm !! i)) (identity (length perm))
