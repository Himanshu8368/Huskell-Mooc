...
add x (Set xs)
  | x `elem` xs = Set xs  -- If the element is already in the set, return the set unchanged
...
average (x :| xs) = (x + sum xs) / fromIntegral (length (x:xs))
...
reverseNonEmpty (x :| xs) = case reverse (x:xs) of (y:ys) -> y :| ys
...
instance Ord a => Semigroup (Set a) where
  (Set xs) <> (Set ys) = Set (mergeUnique xs ys)
    where
      mergeUnique [] ys' = ys'
      mergeUnique xs' [] = xs'
...
data Operation1 = Add1 Int Int
                | Subtract1 Int Int
                | Multiply1 Int Int  -- Added multiplication case
  deriving Show

compute1 :: Operation1 -> Int
compute1 (Add1 i j) = i + j
compute1 (Subtract1 i j) = i - j
compute1 (Multiply1 i j) = i * j  -- Implementation for multiplication
...
data Multiply2 = Multiply2 Int Int  -- Added multiplication case
  deriving Show

class Operation2 op where
  compute2 :: op -> Int
  show2 :: op -> String  -- Added show2 method to the typeclass
...
passwordAllowed pw (DoesNotContain chars) = not (any (`elem` chars) pw)
...
render (Operation "+" a1 a2) = "(" ++ render a1 ++ "+" ++ render a2 ++ ")"
render (Operation "*" a1 a2) = "(" ++ render a1 ++ "*" ++ render a2 ++ ")"
