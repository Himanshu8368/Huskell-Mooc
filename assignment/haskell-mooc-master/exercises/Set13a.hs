...
balance accountName = BankOp $ \(Bank b) ->
  let currentBalance = Map.findWithDefault 0 accountName b
  in (currentBalance, Bank b)
...
paren c = do
  currentState <- get
  if currentState < 0 
    then put currentState 
    else updateState c

  where
    updateState '(' = modify (+ 1)
    updateState ')' = modify (subtract 1)
...
count x = do
  items <- get
  case lookup x items of
    Nothing -> put ((x, 1) : items)
    Just _  -> modify (incrementCount x)
  where
    incrementCount y = map (\pair@(a, n) -> if a == y then (a, n + 1) else pair)
