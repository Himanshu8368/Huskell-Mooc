...
isSorted xs = property $ isSortedList xs
...
sumIsLength input output = property $ sumCounts == length input
  where
    sumCounts = sum (map snd output)
...
-- here we make change in inputInOutput function

inputInOutput input output =
  forAll (elements input) $ \x ->
    case lookup x output of
      Just count -> count > 0
      Nothing    -> False
...
genList = do
  ...
  len <- choose (3, 5)
  -- Generate a list of that length with elements between 0 and 10
  unsortedList <- vectorOf len (choose (0, 10))
  -- Return the sorted version of the generated list
...
instance Arbitrary Expression where
  arbitrary = do
    op1 <- arbitrary :: Gen Arg
    op2 <- arbitrary :: Gen Arg
    oneof
      [ return $ Plus op1 op2
