...
rgb Green = [0, 1, 0]
rgb Blue = [0, 0, 1]
rgb (Mix c1 c2) = mixRGB (rgb c1) (rgb c2)
rgb (Invert c) = invertRGB (rgb c)

--helper function 
mixRGB :: [Double] -> [Double] -> [Double]
mixRGB rgb1 rgb2 = [ (r1 + r2) / 2 | (r1, r2) <- zip rgb1 rgb2 ]

invertRGB :: [Double] -> [Double]
invertRGB rgbVals = [ 1 - x | x <- rgbVals ]
------------------------------------------------------------------------------
...
-- Convert a binary number to a human-readable binary string
...
fromBin b = fromBin' b 0
  where
    fromBin' :: Bin -> Int -> Int
    fromBin' End     _ = 0
    fromBin' (O x) n = fromBin' x (n + 1)
    fromBin' (I x) n = (2 ^ n) + fromBin' x (n + 1)
