...
--statements xs ys = liftA2 (\x y -> x ++ " is " ++ y) xs ys
--                ++ liftA2 (\x y -> x ++ " is not " ++ y) xs ys

-- | Combines elements from two lists to create statements.
statements :: [String] -> [String] -> [String]
statements xs ys =
  let combine x y = [x ++ " is " ++ y, x ++ " is not " ++ y]
  in concatMap (uncurry combine) (zip xs ys)
...
-- | A simple calculator that performs operations based on input strings.
calculator :: String -> String -> Maybe Int
calculator op num = liftA2 ($) (operation op) (readMaybe num)

-- | Defines the supported operations.
operation :: String -> Maybe (Int -> Int)
operation "negate" = Just negate
operation "double" = Just (*2)
operation _        = Nothing
...
validateAddress streetName streetNumber postCode =
  Address <$> checkLength streetName
          <*> checkDigits streetNumber
          <*> checkPostcodeFormat postCode

checkLength :: String -> Validation String
checkLength s = check (length s <= 20) "Invalid street name" s

checkDigits :: String -> Validation String
checkDigits s = check (all isDigit s) "Invalid street number" s

checkPostcodeFormat :: String -> Validation String
checkPostcodeFormat s = check (all isDigit s && length s == 5) "Invalid postcode" s
...
normalizePhone s =
  let cleanedInput = filter (/= ' ') s
  in checkLength cleanedInput *> validateDigits cleanedInput
  where
    checkLength x = check (length x <= 10) "Too long" x
    validateDigits x = traverse checkDigit x
      where
...
instance Applicative Priced where
  pure x = Priced 0 x
  Priced p1 f <*> Priced p2 a = Priced (p1 + p2) (f a)
...
instance (Applicative f, Applicative g) => Applicative (Both f g) where
  ...
  Both f <*> Both a = Both (liftA2 (<*>) f a)
  liftA2 h (Both a) (Both b) = Both (liftA2 (liftA2 h) a b)
