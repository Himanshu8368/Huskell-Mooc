...
sums i = helper 1 0 i
  where
    ...
    helper n acc maxN
      | n > maxN  = []
      | otherwise = let newAcc = acc + n
                    in newAcc : helper (n + 1) newAcc maxN
...
sumsOfHelper [] _ = []
sumsOfHelper (x:xs) acc = let currentSum = acc + x
                          in currentSum : sumsOfHelper xs currentSum
...
mymaximum bigger initial (x:xs) =
    let maxRest = mymaximum bigger initial xs
    in if bigger x maxRest then x else maxRest
