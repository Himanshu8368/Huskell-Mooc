...
greetText name
  | T.length name > 15 = T.pack "Hello, " `mappend` T.take 15 name `mappend` T.pack "...!"
  | otherwise          = T.pack "Hello, " `mappend` name `mappend` T.pack "!"
...
takeStrict n = T.concat . TL.toChunks . TL.take n
...
byteRange bs
  | B.null bs = 0
  | otherwise = maximum bytes - minimum bytes
  where
    bytes = B.unpack bs
...
pingpong bs = BL.cycle $ BL.fromChunks [bs, B.reverse bs]
