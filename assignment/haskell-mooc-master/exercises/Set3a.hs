...
whileRight check x = 
  case check x of
    Left result  -> result
    Right nextX  -> whileRight check nextX
...
interpret x y (cmd:cmds) results
  | cmd == "up"     = interpret x (y + 1) cmds results
  | cmd == "down"   = interpret x (y - 1) cmds results
  | cmd == "left"   = interpret (x - 1) y cmds results
  | cmd == "right"  = interpret (x + 1) y cmds results
  | cmd == "printX" = interpret x y cmds (show x : results)
  | cmd == "printY" = interpret x y cmds (show y : results)
  | otherwise       = interpret x y cmds results
