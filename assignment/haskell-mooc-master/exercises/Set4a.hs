...
longest :: Ord a => [[a]] -> [a]
longest [] = error "Empty list of lists"
longest [x] = x
longest (x:xs) = foldr (\list acc -> if length list > length acc || (length list == length acc && head list < head acc) then list else acc) x xs
...
incrementKey key ((k,v):kvs)
  | key == k   = (k, v + 1) : incrementKey key kvs
  | otherwise  = (k, v) : incrementKey key kvs
...
freqs xs = foldr (\x -> Map.alter (Just . maybe 1 (+1)) x) Map.empty xs
...
transfer from to amount bank
  | amount <= 0 = bank  -- Do not transfer if amount is negative or zero
  | Map.notMember from bank || Map.notMember to bank = bank  -- Do not transfer if either account doesn't exist
  | bank Map.! from < amount = bank  -- Do not transfer if from account doesn't have enough money
  | otherwise = Map.adjust (\bal -> bal - amount) from $ Map.adjust (\bal -> bal + amount) to bank
...
maxIndex arr = fst $ maximumBy (\(_,v1) (_,v2) -> compare v1 v2) (assocs arr)
