...
prettyPrint size queens = unlines [row r | r <- [1..size]]
  where
    row r = [if (r, c) `elem` queens then 'Q' else '.' | c <- [1..size]]
...
danger _ [] = False
danger candidate (queen:queens) =
  sameRow candidate queen ||
  sameCol candidate queen ||
  sameDiag candidate queen ||
  sameAntidiag candidate queen ||
...
prettyPrint2 size queens = unlines [ [mark (i, j) | j <- [1..size]] | i <- [1..size] ]
  where
    mark coord
      | coord `elem` queens = 'Q'
      | danger coord queens = '#'
...
fixFirst n (q@(i, j):qs)
  | j > n = Nothing
  | not (danger q qs) = Just (q:qs)
  | otherwise = fixFirst n ((i, j + 1) : qs)
