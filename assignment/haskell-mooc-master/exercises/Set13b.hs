...
neighbors maze place = case lookup place maze of
                         Just ns -> ns
                         Nothing -> []
...
visit maze place = do
  ...
  unless (place `elem` visited) $ do
    put (place : visited)
    mapM_ (visit maze) (neighbors maze place)
...
path maze place1 place2 = evalState (do
  visit maze place1
  visited <- get
  return (place2 `elem` visited)) []
